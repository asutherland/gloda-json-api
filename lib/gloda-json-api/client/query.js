...
var Unmarshaller = {
  ...
  _unmarshal: function(id, marshalled, nounType, inStruct, outDicts) {
    ...
    for (var attrKey in schema) {
      var attrSchema = schema[attrKey];
      if (!attrSchema.reference) {
        if (attrSchema.singular)
          outObj[attrKey] = marshalled[attrKey];
        else
          outObj[attrKey] = marshalled[attrKey].concat();
      }
      else {
        var subid, subschema = inStruct.nounSchemas[attrSchema.nounType];
        var subdict = inStruct.nounValues[attrSchema.nounType];
        if (attrSchema.singular) {
          subid = marshalled[attrKey];
          if (!(subid in outDicts[attrSchema.nounType]))
            this._unmarshal(subid, subdict[subid], attrSchema.nounType,
                            inStruct, outDicts);
          outObj[attrKey] = outDicts[attrSchema.nounType][subid];
        }
        else {
          ...
          for (var i = 0; i < subin.length; i++) {
            ...
            if (!(subid in outDicts[attrSchema.nounType]))
              this._unmarshal(subid, subdict[subid], attrSchema.nounType,
                              inStruct, outDicts);
            subout.push(outDicts[attrSchema.nounType][subid]);
          }
        }
      }
    }
  ...
  unmarshalGlodaNounInstances: function(marshalledSet) {
    ...
    for (var i = 0; i < inItems.length; i++) {
      if (!(inItems[i] in outDict))
        this._unmarshal(inItems[i], inDict[inItems[i]], nounType,
                        marshalledSet, outDicts);
...
var BaseProxyQuery = {
  getCollection: function(listener) {
    ...
    this._gloda._doRemote("query", queryPayload, function(result) {
      var collection = new ProxiedCollection(listener, result);
    });
  },
};
